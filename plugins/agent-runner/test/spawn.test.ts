...
import { applyEvent, buildAgentArgs, emptyStreamState } from "../src/executor.ts";
import { forwardedExecArgs, jsonRunArgs, resolvePiInvocation } from "../src/spawn.ts";

describe("applyEvent", () => {
  test("adds separate assistant replies while ignoring the agent_end replay", () => {
    const state = emptyStreamState();
    const first = { role: "assistant", content: [{ type: "text", text: "first" }], usage: { input: 11, output: 2, totalTokens: 13 } };
    const second = { role: "assistant", content: [{ type: "text", text: "second" }], usage: { input: 13, output: 3, totalTokens: 16 } };
    applyEvent(state, { type: "message_end", message: first });
    applyEvent(state, { type: "message_end", message: second });
    applyEvent(state, { type: "agent_end", messages: [first, second] });
    expect(state.usage.totalTokens).toBe(29);
    expect(state.usage.input).toBe(24);
  });
  ...
  test("usage is cumulative, so a zeroed later event does not erase a total", () => {
    const state = emptyStreamState();
    applyEvent(state, { type: "message_update", usage: { input: 100, output: 20, totalTokens: 120 } });
...
describe("buildAgentArgs", () => {
  test("orders flags and keeps the prompt after --", () => {
    const args = buildAgentArgs({
      invocation: { command: "pi", args: [] },
      extensionPaths: ["/tmp/guard.ts"],
      tools: ["read", "bash"],
  ...
  test("omits every optional flag when none is given", () => {
    const args = buildAgentArgs({ invocation: { command: "pi", args: [] }, prompt: "hi" });
    expect(args).toEqual(["--mode", "json", "-p", "--no-session", "--", "hi"]);
